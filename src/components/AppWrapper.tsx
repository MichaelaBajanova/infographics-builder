...
class AppWrapper extends React.Component<{}, IState> {
    ...
    private handleDivideSection = (id: number) => {
        ...
        const newSectionLayout = produce(sectionLayout, draftSectionLayout => {
            ...
            if (draftSectionLayout[newLayoutSection.y].length > infographicsDetails.columns) {
    ...
    //
    //     const sectionToBeDivided = findSectionInLayout(id, sectionLayout)
    //     // even though findSectionInLayout should not return undefined, this needs to be done to get rid off errors
    //     if (!sectionToBeDivided) return
    //
    //     const newSection: IInfographicsSection = createNewSection(lastUsedId)
    //
    //     let newSectionWidth: number = (sectionToBeDivided.end - sectionToBeDivided.start) / 2 // dividing in half
    //     const dividedSectionUpdated: ILayoutSection = {
    //         id: sectionToBeDivided.id,
    //         x: sectionToBeDivided.x,
    //         y: sectionToBeDivided.y,
    //         start: sectionToBeDivided.start,
    //         end: sectionToBeDivided.start + newSectionWidth,
    //         spanColumns: 1,
    //         columnStart: sectionToBeDivided.columnStart,
    //         columnEnd: sectionToBeDivided.columnEnd,
    //     }
    //
    //     const newLayoutSection: ILayoutSection = {
    //         id: lastUsedId,
    //         x: sectionToBeDivided.x + 1,
    //         y: sectionToBeDivided.y,
    //         start: dividedSectionUpdated.end,
    //         end: dividedSectionUpdated.end + newSectionWidth,
    //         spanColumns: 1,
    ...
    //     }
    //
    //     let sectionLayoutCopy = [...sectionLayout]
    //     // replace old section with new divided one
    //     sectionLayoutCopy[sectionToBeDivided.y][sectionToBeDivided.x] = dividedSectionUpdated
    //
    //     // we need to move all sections that come after divided section to coordinate x+1
    //     for (let i = sectionToBeDivided.x + 1; i < sectionLayout[sectionToBeDivided.y].length; ++i) {
    //         ++sectionLayout[sectionToBeDivided.y][i].x
    //     }
    //
    //     // and add new section after the one being divided
    //     sectionLayoutCopy[sectionToBeDivided.y].splice(sectionToBeDivided.x + 1, 0, newLayoutSection)
    //     // if row that is being modified will as a result have more columns than current number of columns in infographics
    //     // detail, then every section on coordinate x = 0 that is not in this row should take up one more column
    //     if (sectionLayoutCopy[newLayoutSection.y].length > infographicsDetails.columns) {
    //         sectionLayoutCopy.forEach(row => {
    //             if (row[0].y !== newLayoutSection.y) {
    //                 ++row[0].columnEnd
    //             }
    //         })
    //     }
    //
    //     const increaseNumberOfCols = sectionLayoutCopy[newLayoutSection.y].length > infographicsDetails.columns
    //
    //     this.setState({
    //         infographicsDetails: {
    //             width: infographicsDetails.width,
    //             sections: [...infographicsSections, newSection],
    //             sectionLayout: sectionLayoutCopy,
    //             columns: increaseNumberOfCols ? ++infographicsDetails.columns : infographicsDetails.columns,
    //         },
    //         lastUsedId: lastUsedId + 1,
    //     });
